...
- **Sync vs. transfer (SQLite-tracked).** Two distinct ops, mirroring `movement_logger_desktop` (issues #3/#4): per-file **Download** (manual, unchanged) and **Sync now** (pull every session file not already mirrored, remember what was pulled). `ble/SyncDb.kt` is the port of the desktop's `sync_db.rs` — same `synced_files` schema, PK `(box_id, name, size)` (`size` in the key so a regrown same-name file re-pulls). `box_id` = BLE MAC, captured in `FileSyncCore.connect()`. DB at `filesDir/sqlite/sync.db` — internal `filesDir`, *not* the external `getExternalFilesDir` download folder (user-clearable; desktop analogue is "anchored to $HOME, not the download folder"), own `sqlite/` subdir per issue #4. Uses `android.database.sqlite.SQLiteDatabase` directly — no Room / no new Gradle dep. Flow: `syncNow()` sets a pending flag → fresh LIST → `ListDone` runs the diff only when the flag is set → `isSensorData` files not in the DB drain serially through the existing single-op `download()` path (BLE worker is one-op-at-a-time; queue advanced from `ReadDone`). Manual downloads also `markSynced`. **Policy (locked): purely additive — never DELETE.** Only `Sens*/Gps*/Bat*.csv` + `Mic*.wav` auto-sync. iOS (`BLE/SyncDb.swift`, system `SQLite3`, `Application Support/sqlite/sync.db`, box_id = `CBPeripheral.identifier`) is the exact peer.
